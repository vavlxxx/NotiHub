...
class UserService(BaseService):
    ...
    async def add_user(
        self, user_data: RequestRegisterUserDTO, user_meta: dict | None = None
    ) -> None:
    ...
    async def login_user(
        self, user_data: RequestLoginUserDTO, response: Response, request: Request
    ):
        try:
            user: UserWithPasswordDTO = await self.db.users.get_user_with_passwd(
    ...
    async def get_user(
        self, *filter, include_channels: bool = True, **filter_by
    ) -> UserDTO | UserWithChannelsDTO:
        try:
        ...
        if include_channels:
            channels: list[ChannelDTO] = await self.db.channels.get_all_filtered(
                user_id=user.id
            )
            user_: UserWithChannelsDTO = UserWithChannelsDTO(
                **user.model_dump(), channels=channels
            )
            return user_
        return user
