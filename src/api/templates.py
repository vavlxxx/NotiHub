...
router = APIRouter(prefix="/templates", tags=["Управление шаблонами для уведомлений"])


@router.get(
    "", summary="Получить список всех шаблонов", dependencies=[Depends(auth_required)]
)
@cache(expire=120)
...
async def add_template(
    db: DBDep,
    user_meta: UserMetaDep,
    data: RequestAddTemplateDTO = Body(
        description="Данные о шаблоне", openapi_examples=EXAMPLE_TEMPLATES
    ),
):
    try:
        template = await TemplateService(db).add_template(
...
async def update_template(
    ...
    user_meta: UserMetaDep,
    template_id: int = Path(description="ID шаблона"),  # type: ignore
    data: TemplateUpdateDTO = Body(
        description="Данные о шаблоне", openapi_examples=EXAMPLE_TEMPLATES
    ),
):
    try:
        await TemplateService(db).update_template(
            template_id=template_id, user_meta=user_meta, data=data
        )
    except TemplateNotFoundError as exc:
        raise TemplateNotFoundHTTPError from exc
    except CategoryNotFoundError as exc:
...
async def delete_template(
    ...
    try:
        await TemplateService(db).delete_template(
            template_id=template_id, user_meta=user_meta
        )
    except TemplateNotFoundError as exc:
        raise TemplateNotFoundHTTPError from exc
    except ValueOutOfRangeError as exc:
