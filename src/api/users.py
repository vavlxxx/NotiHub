...
async def register_user(
    ...
    user_data: RequestRegisterUserDTO = Body(
        description="Логин и пароль", openapi_examples=EXAMPLE_USER_LOGIN
    ),
):
    try:
...
async def login_user(
    ...
    user_data: RequestLoginUserDTO = Body(
        description="Логин и пароль", openapi_examples=EXAMPLE_USER_LOGIN
    ),
):
    try:
        access_token, expire = await UserService(db).login_user(
            response=response, request=request, user_data=user_data
        )
    ...
    return {"status": "OK", "access_token": access_token, "expires_in": expire}
...
async def edit_user(
    ...
    user_data: UserUpdateDTO = Body(
        description="Данные о пользователе", openapi_examples=EXAMPLE_USER_UPDATE
    ),
):
    try:
...
@router.post(
    path="/logout", summary="Выйти из аккаунта", dependencies=[Depends(auth_required)]
)
