...
async def test_user_register(
    username: str, password: str, expected_sc: int, ac: AsyncClient
):
    resp = await ac.post(
...
async def test_register_new_admin(admin: AsyncClient):
    resp = await admin.post(
        "/auth/register",
        json={"username": "admin2", "password": "admin123456789", "role": "ADMIN"},
    )
    ...
    resp = await admin.post(
        "/auth/login", json={"username": "admin2", "password": "admin123456789"}
    )
    assert resp and resp.status_code == 200
